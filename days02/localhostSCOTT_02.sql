-- scott계정이 소유하고 있는 테이블 조회 (검색)
select * from tabs;

--

select EMPNO, ENAME, EMP.JOB from EMP, DEPT;
--eco???실행하는동안 불필요한 다른것들은 꺼놨다가 끝나면 다시 킴

--SQL
--quert( select문 ) -> 하나 이상의 테이블 또는 뷰로 부터 데이터를 가져오는 sql문
...
from all_tables;-- s
from ALL_users;
from DBA_users; --SCOTT은 볼수 없음
from USER_users; --테이블 or 뷰
--
--SCOTT이 소유하고 있는 4개의 테이블 정보 출력(검색, 조회)
SELECT *
FROM TABS; --user_tables의 동의어
from user_tablse; --scott이 만든 모든 테이블
form all_tablse;-- 모든 테이블 정보
...
from emp;
-- 중복 제거[distinct]
--사원번호, 사원명, 기본급 , 커미션, 기본급 + 커미션 월급/ 연봉
select empno, ename, sal,comm 
                  --    ,sal+comm as "my pay" --띄어쓰기 포함 별칭을 큰 따옴표로 묶어주세욥
                  --  ,sal+comm as "pay"
                  --  ,sal+comm as pay
                    ,sal+comm 월급
                -- comm null인경우 0으로 처리해서 월급 계산
                --오라클 null처리는 어떻게 하는가?
                -- NVL(expr1,expr2) : null 값을 원하는 데이터 값으로 바꾸어 주는 함수 
from emp;

SELECT ENAME,COMM
        ,NVL(COMM,0) a 
        -- ,NVL(to_ChaR(COMM),'널값') b --오라클 날짜, 문자열 ''
...
FROM EMP;
--[조건]
--10번 부서원의 정보(부서번호, 사원번호, 사원명, 입사일자) 출력
select deptno,empno, ename, hiredate
from emp

where not(deptni =10) not부정연산자.
        --where !=,!=,<>
      deptno=10; 
      deptno=!10;
      deptno<>!10;
...
select num empno, name ename, city
from insa
--where city = '경기' or city = '서울' or city = '인천'; --수도권인 사람들
where city != '경기' and city != '서울' and city != '인천'; --수도권이 아닌사람들
where city not in('서울','경기','인천');

where not city in('서울','경기','인천');--다름 in에 걸리는 not
